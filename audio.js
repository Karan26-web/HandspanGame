...
HS.Audio = (function () {
  ...
  // The only real audio asset is the hand-place drop sound (assets/handPlaceSound.webm),
  // played through an <audio> element (see playHandPlace) so it also works on file://.
  var FILES = {};
  ...
  // Hand-place drop sound — the one real audio asset (WebM/Opus). Uses an
  // <audio> element (not fetch/WebAudio) so it plays on file:// too.
  var handPlaceEl = null;
  function playHandPlace() {
    if (muted) return;
    try {
      if (!handPlaceEl) {
        handPlaceEl = new Audio('assets/handPlaceSound.webm');
